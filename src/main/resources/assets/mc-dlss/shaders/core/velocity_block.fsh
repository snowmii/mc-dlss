...
// Moving-block velocity fragment shader: the vanilla core/block fragment shader - which is
// what the piston moving-block pipelines SOLID_BLOCK and CUTOUT_BLOCK bind - plus the
// velocity-MRT payload write.
//
// The moving-block velocity twin swaps this shader in for the source's core/block fragment
// shader, so the color output must be byte-identical to vanilla's. The body below is the
// vanilla block.fsh verbatim (Sampler0 sampling, vertex color, ColorModulator, the
// ALPHA_CUTOUT discard, and apply_fog); the additions are the BlockVelocityConfig uniform
// block, the velocityColor output, and the per-pixel object-motion computation at the end
// of main().
//
...
// ObjectReprojection maps this frame's jittered clip position - what the rendered moving block
// and its reversed-Z depth actually hold - to where that same block surface sat in the
// previous frame: the camera's jitter-stripped reprojection with the block's own piston
// offset-delta displacement conjugated into it. VelocityParams.x is the reset/unknown-history
// flag; yz is the scene velocity viewport size, which is what gl_FragCoord is sized to.
layout(std140) uniform BlockVelocityConfig {
...
out vec4 fragColor;
// The moving-block velocity twin's second color target: NDC object motion. This shader is
// only ever bound on the velocity twin, whose second attachment this output writes; on the
// one-target vanilla pipeline the vanilla core/block shader is bound instead.
...
// rather than the identity-derived zero (which would read as a block that stood still) or an
...
void main() {
    ...
    // NDC object motion, the payload DLSS reads for the moving block this fragment drew.
    //
    ...
    // The reset/unknown-history flag - a frame with no valid predecessor - forces every pixel
    // invalid on top of that, so a first observation never reads as the identity-derived zero.
    ...
    if (!invalidPixel) {
        ...
        // motion spans a few NDC units, so the sentinel magnitude is a wide safety margin, not
        // a bound any valid vector approaches.
