...
// Weather velocity fragment shader: the vanilla core/particle fragment shader - which is what
// the weather pipelines WEATHER_DEPTH_WRITE and WEATHER_NO_DEPTH_WRITE bind - plus the
// velocity-MRT payload write.
//
// The weather velocity twin swaps this shader in for the source's core/particle fragment
// shader, so the color output must be byte-identical to vanilla's. The body below is the
// vanilla particle.fsh verbatim (Sampler0 sampling, vertex color, ColorModulator, the
// < 0.1 alpha discard, and apply_fog); the additions are the VelocityConfig uniform block,
// the velocityColor output, and the per-pixel camera-motion computation at the end of main().
//
...
// writer's existing VelocityConfig payload: the weather writer fills the same block on the
// same command encoder, so the weather pass reads the same camera motion and reset semantics
// the terrain pass does.
...
out vec4 fragColor;
// The weather velocity twin's second color target: jitter-free NDC camera motion. This shader
// is only ever bound on the velocity twin, whose second attachment this output writes; on the
// one-target vanilla weather pipeline the vanilla particle shader is bound instead.
...
void main() {
    ...
    // Jitter-free NDC camera motion, the payload DLSS reads for the weather this fragment drew.
    //
