...
class AcceptanceRecordTest {
	...
	fun `internal resolution reads unavailable when NGX never chose one`() {
		...
		assertTrue(record.contains("internal-resolution=${DlssAcceptanceRecord.UNAVAILABLE}"), record)
	...
	fun `unreadable minecraft build falls back to the reviewer rather than vanishing`() {
		...
		assertTrue(record.contains("minecraft-build=${DlssAcceptanceRecord.REVIEWER_SUPPLIED}"), record)
	}
