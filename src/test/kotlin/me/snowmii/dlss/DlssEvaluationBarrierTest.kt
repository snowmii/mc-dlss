...
class DlssEvaluationBarrierTest {
		...
		HeadlessVulkanFixture(
			...
			DlssNative.open(library).use { native ->
				...
				val evaluated = adapter.evaluate(
					evaluationRequest(commandBuffer.address(), color, depth, images!!),
				)
				...
				assertTrue(
					adapter.evaluate(evaluationRequest(secondFrame.address(), color, depth, images)),
	...
	): DlssEvaluationRequest = DlssEvaluationRequest(
		commandBuffer = commandBuffer,
		colorView = color.view(),
		colorImage = color.image(),
		colorFormat = color.format(),
		colorAspectMask = color.aspectMask(),
		colorLevelCount = 1,
		colorLayerCount = 1,
		depthView = depth.view(),
		depthImage = depth.image(),
		depthFormat = depth.format(),
		depthAspectMask = depth.aspectMask(),
		depthLevelCount = 1,
		depthLayerCount = 1,
		motionView = images.motionView,
		motionImage = images.motionImage,
		motionFormat = images.motionFormat,
		motionAspectMask = VK_IMAGE_ASPECT_COLOR_BIT,
		motionLevelCount = 1,
		motionLayerCount = 1,
		outputView = images.outputView,
		outputImage = images.outputImage,
		outputFormat = images.outputFormat,
