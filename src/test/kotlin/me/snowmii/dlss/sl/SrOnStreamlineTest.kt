...
class SrOnStreamlineTest {
	...
	@Order(3)
	@Test
	fun `extension queries and initialize come from Streamline alone`() {
		...
		// retired discovery fallback no longer answers a query that ran before bootstrap.
		val instanceQuery = apiSource
			.substringAfter("mc_dlss_query_instance_extension")
			.substringBefore("mc_dlss_query_device_extension")
		assertTrue(instanceQuery.contains("collect_streamline_extensions(false"))
		val deviceQuery = apiSource
			.substringAfter("mc_dlss_query_device_extension")
			.substringBefore("mc_dlss_query_device_feature_12")
		assertTrue(deviceQuery.contains("collect_streamline_extensions(true"))

		// initialize validates and records only the live Vulkan tuple: the handles go into the
		// state the module-owned images and motion pass use, bootstrap + proxy activation are
		// required (an initialize before either fails and records nothing), the recorded tuple
		// must be the one proxy activation handed to slSetVulkanInfo, and re-initialization
		// against a different tuple is refused rather than re-recorded.
		val initialize = apiSource
			.substringAfter("mc_dlss_initialize")
			.substringBefore("mc_dlss_query_optimal_dimensions")
		listOf(
			...
			"g_state.streamlineInitialized",
			"g_state.proxyDevice == 0",
			"g_state.proxyInstance != vk_instance",
		).forEach { fragment ->
			assertTrue(initialize.contains(fragment), "initialize must contain: $fragment")
		}
		// The compatibility paths are accepted and ignored, never stored.
		assertTrue(!initialize.contains("g_state.sdkPath"))
		assertTrue(!initialize.contains("g_state.dataPath"))

		// Nothing on the option/evaluate seams depends on a direct-NGX session: the SL session
		// is the one gate the configure and evaluate paths run against.
		val slSource = Files.readString(Path.of("native", "internal", "sl_dlss.cpp"))
