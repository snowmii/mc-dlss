...
class AcceptanceScaffoldTest {
	...
	fun `the document names the properties the code reads`() {
		listOf(
			DlssStartupConfig.ENABLED_PROPERTY,
			DlssStressConfig.ENABLED_PROPERTY,
			DlssStressConfig.STEPS_PROPERTY,
			DlssStressConfig.OCTAVES_PROPERTY,
			DlssStressConfig.GODRAYS_PROPERTY,
			DlssStressConfig.INTENSITY_PROPERTY,
		).forEach { property ->
	...
	fun `the document names the diagnostic lines the reviewer reads values off`() {
		...
		assertTrue(document.contains(DlssAcceptanceRecord.HEADING.substringBefore(" (")))
		assertTrue(document.contains("DLSS first world phase"))
		assertTrue(document.contains("DLSS world frame rate"))
		assertTrue(document.contains(DlssAcceptanceRecord.REVIEWER_SUPPLIED))
	}

	@Test
	fun `the workload bounds the document quotes are the ones the config enforces`() {
		val extreme = java.util.Properties().apply {
			setProperty(DlssStressConfig.STEPS_PROPERTY, "999999")
			setProperty(DlssStressConfig.OCTAVES_PROPERTY, "999999")
			setProperty(DlssStressConfig.GODRAYS_PROPERTY, "999999")
			setProperty(DlssStressConfig.INTENSITY_PROPERTY, "999999")
		}
		val clamped = DlssStressConfig.from(extreme)
		val defaults = DlssStressConfig.from(java.util.Properties())
