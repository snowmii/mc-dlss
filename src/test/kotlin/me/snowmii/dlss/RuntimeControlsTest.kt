...
class RuntimeControlsTest {
	...
	fun `a mode change hands the new render dimensions to the native bridge before the next frame`() {
		...
		// DlssFrameEvaluation, which needs a live command encoder to have acquired anything, so
	...
	fun `the acceptance record names the preset the session is actually running`() {
		...
		val record = DlssAcceptanceRecord.render(
			minecraftBuild = "26.2",
	...
	private inner class Fixture {
		...
		private val phase = DlssWorldPhase(
			runtime = runtime,
			present = { _, _ -> },
	...
	private class FakeNative : DlssNativeApi {
		...
		fun renderFor(mode: DlssQualityMode): DlssDimensions = when (mode) {
			DlssQualityMode.DLAA -> DlssDimensions(2560, 1440)
			DlssQualityMode.QUALITY -> DlssDimensions(1280, 720)
			DlssQualityMode.BALANCED -> DlssDimensions(1485, 835)
			DlssQualityMode.PERFORMANCE -> DlssDimensions(1706, 960)
			DlssQualityMode.ULTRA_PERFORMANCE -> DlssDimensions(853, 480)
		}
		...
		override fun queryOptimalDimensions(
			...
			outputHeight: Int,
			qualityMode: Int,
		): DlssDimensions = renderFor(DlssQualityMode.entries.first { it.ngxValue == qualityMode })
		...
		override fun configure(
			...
			if (configureResult != DlssNativeApi.SUCCESS_RESULT) {
