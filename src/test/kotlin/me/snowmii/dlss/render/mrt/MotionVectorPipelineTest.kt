 ...
 * the fill on VELOCITY_MRT and the compute writer on CAMERA_ONLY. [me.snowmii.dlss.render.mrt.velocityTwin] remains
...
class MotionVectorPipelineTest {
	...
	fun `a session without DLSS keeps terrain passes vanilla`() {
		val phase = velocityWorldPhase(velocityRuntime(enabled = false))
	...
	fun `a phase without a velocity companion keeps terrain passes vanilla`() {
		val phase = velocityWorldPhase(velocityRuntime(withVelocity = false))
	...
	fun `first foreign terrain pipeline latches camera-only and drops the velocity view`() {
		...
		phase.observePipeline(pipelineObservation(RenderPipelines.SOLID_TERRAIN))
