...
class WorldTargetRoutingTest {
	...
	fun `vanilla routes keep world and main target at output dimensions`(case: VanillaCase) {
		val runtime = runtime(case.session())

		val worldTarget = runtime.beginWorldPhase(
			normalInWorldFrame = case.normalInWorldFrame,
			outputDimensions = case.outputDimensions,
		)
		val route = runtime.activeRoute

		assertNull(worldTarget)
		// A session that never started publishes no route - the frame just renders vanilla into
		// the main target. A started session that still routes vanilla publishes it.
		if (route != null) {
	...
	/** Startup is pre-resolved so only the routing decision is under test. */
	private fun runtime(session: DlssSession) = RenderRuntime(
		session = session,
	 ...
	 */
	private fun worldPhase(session: DlssSession) = WorldPhase(
